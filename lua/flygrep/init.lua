...
local function get_file_pos(line)
  local filename = vim.fn.fnamemodify(
    vim.fn.fnameescape(vim.fn.split(line, [[:\d\+:]])[1]),
    ':p'
  )
  local linenr =
...
local function apply_to_quickfix()
  ...
  local searching_result =
    vim.api.nvim_buf_get_lines(result_bufid, 0, -1, false)
  local searching_partten = vim.api.nvim_buf_get_lines(
    prompt_bufid,
    0,
    -1,
    false
...
local function open_win()
  ...
  vim.api.nvim_create_autocmd({ 'TextChangedI' }, {
    ...
    callback = function(ev)
      ...
      if grep_input ~= '' then
        ...
        vim.api.nvim_buf_set_lines(result_bufid, 0, -1, false, {})
        if
          config.enable_preview and vim.api.nvim_buf_is_valid(preview_bufid)
        then
          vim.api.nvim_buf_set_lines(preview_bufid, 0, -1, false, {})
        end
      end
  ...
  vim.keymap.set(
    'i',
    config.mappings.next_item,
    next_item,
    { buffer = prompt_bufid }
  )
  vim.keymap.set(
    'i',
    config.mappings.previous_item,
    previous_item,
    { buffer = prompt_bufid }
  )
